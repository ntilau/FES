...
% Assemble harmonic-balance system with Kerr (cubic) nonlinearity
% using FFT-based coupling of the electric field to epsr.
...
%% Assembly -- element loop with Kerr update
...
for ie = 1:Mesh.NELE
    ...
    if Mesh.elab(ie) == Mesh.NLlab
        ...
        Mtrl.func = @(x)(Mesh.epsr(Mesh.elab(ie)) ...
                         + Mesh.kerr(Mesh.elab(ie)) * abs(x).^2);
...
%% Boundary conditions (ABC, DD, Dir, Neu, WP) -- same pattern as AssembHB
if FlagABC
