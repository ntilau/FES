...
% Assemble harmonic-balance system with Kerr nonlinearity using
% analytic (symbolic) coupling matrices rather than FFT.
...
for ie = 1:Mesh.NELE
    ...
    if Mesh.elab(ie) == Mesh.NLlab
        ...
        E_vals  = zeros(nHarms, 1);
        for jh = 1:nHarms
            sol = Sys.u0(gIs + (jh - 1) * Sys.NDOFs);
            Field.E(2 * jh - 1, 1) = Shape2(.5, .5) * sol;
            E_vals(jh) = Field.E(2 * jh - 1, 1);
        end
        if isfield(Sys, 'FFT')
            Mtrl.func = @(x)(Mesh.epsr(Mesh.elab(ie)) ...
                             + Mesh.kerr(Mesh.elab(ie)) * x.^2);
            [Mtrl] = GetECouplFFT(Mtrl, Field);
            if isfield(Sys, 'SinOnly') && Sys.SinOnly
                EpsrC = Mtrl.D(1:2:2 * (Mtrl.nHarms), 1:2:2 * (Mtrl.nHarms));
            else
                EpsrC = Mtrl.D;
            end
        else
            [EpsrC] = GetCouplKerrAnalyt(nHarms, abs(E_vals), ...
...
%% Boundary conditions (ABC, DD, Dir, Neu, WP)
if FlagABC
