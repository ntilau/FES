...
% Assemble harmonic-balance FE system for ferrite (gyrotropic)
% materials using FFT-based coupling to update Mur and Kr tensors.
...
%% Harmonics and ferrite material parameters
nHarms = Sys.nHarms;
Ferr   = Mesh.Ferr;
omega  = 2 * pi * Sys.freq;
...
MtrlMur.func = @(x, y, z) (1 + (Ferr.w0 + 1i * Ferr.aDH) .* Ferr.wm ...
    ./ ((Ferr.w0 + 1i * Ferr.aDH).^2 ...
        + sum(((val .* x * y)).^2, 1) - (z .* omega).^2));
MtrlKr.func = @(x, y, z) ((z .* omega) .* Ferr.wm ...
    ./ ((Ferr.w0 + 1i * Ferr.aDH).^2 ...
        + sum(((val .* x * y)).^2, 1) - (z * omega).^2));

%% Assembly -- element loop with ferrite update
...
for ie = 1:Mesh.NELE
    ...
    if Mesh.elab(ie) == Mesh.NLlab
        %% Ferrite region: compute Mur and Kr from H-field via FFT
        MU  = kron(reshape(Mesh.MurMat(:, ie), Sys.nHarms, Sys.nHarms), eye(2)) ...
            + kron(reshape(Mesh.KrMat(:, ie), Sys.nHarms, Sys.nHarms), ...
                   1i * [0 1; -1 0]);
        Bxy = zeros(2, nHarms);
        for jh = 1:nHarms
    ...
    %% Element stiffness and mass per harmonic block
    ...
    for jh = 1:nHarms
        for kh = 1:nHarms
            ...
            T{jh, kh} = zeros(nums);
            for iq = 1:length(wq2)
                S{jh, kh} = S{jh, kh} + detJ ...
                    * ((invJt * dNQuad2{iq})' ...
                       * NurC{jh, kh} * (invJt * dNQuad2{iq})) * wq2(iq);
                T{jh, kh} = T{jh, kh} + detJ ...
                    * (NQuad2{iq}' * EpsrC(jh, kh) * NQuad2{iq}) * wq2(iq);
            end
        end
    end

    %% Assemble harmonic-block matrix
    idx = s:s + nums^2 * nHarms^2 - 1;
    for jh = 1:nHarms
        for kh = 1:nHarms
            for j = 1:nums
                for k = 1:nums
                    tmpIdx = idx(((jh - 1) * nHarms + kh - 1) * nums^2 ...
                                + nums * (j - 1) + k);
...
%% Boundary conditions (identical structure to AssembHB)
if FlagABC
            ...
            if sum(onDir) > 0
                ...
                for i = 1:length(idOnDir)
                    ...
                    for iq = 1:length(wq1)
                        Stt = Stt + norm(l) ...
                            * ((dNQuad1{iq} / detJ).' ...
                               * (dNQuad1{iq} / detJ)) * wq1(iq);
                        Ttt = Ttt + norm(l) ...
                            * (NQuad1{iq}.' * NQuad1{iq}) * wq1(iq);
                    end
                    idx = s:s + (Sys.pOrd + 1)^2 - 1;
                    for j = 1:(Sys.pOrd + 1)
                        for k = 1:(Sys.pOrd + 1)
