...
void option::print_usage(std::ostream& ostr) const
{
    ...
    ostr << "  [+fr $lf $hf $n]             discrete frequency sweep\n";
    ostr << "  [+formula <name>]             select formulation:\n";
    ostr << "       em_e_fd                   3D frequency-domain (default)\n";
    ostr << "       em_ez_fd                  2D TMz (scalar Helmholtz)\n";
    ostr << "       em_e_tl_eig               waveport eigenmodes\n";
    ostr << "       em_e_qs                   electrostatic quasistatic\n";
    ostr << "       em_e_fd_dd                3D EM with domain decomposition\n";
    ostr << "       em_e_fd_nl                3D EM with nonlinear Kerr\n";
    ...
    ostr << "  [+sgl]                       single precision (default: double)\n";
    ostr << "  [+direct]                    direct sparse solver (MUMPS, default)\n";
    ostr << "  [+gmres $tol $restart]       GMRES iterative solver\n";
    ostr << "  [+nl $h $mtrl $kerr $relax]  nonlinear Kerr materials\n";
    ostr << "  [+dd $n]                     domain decomposition in $n regions\n";
    ostr << "            [+gs]              Gauss-Seidel precond. (default)\n";
    ostr << "            [+jc]              Jacobi precond.\n";
    ostr << "  [+field]                     write VTK fields\n";
    ostr << "  [+rad $n_theta $n_phi]       write VTK radiation pattern\n";
    ostr << "Default: +direct +p 1\n";
    ostr << "Example: FES WR90 1e10 +poly AafeeQ +p 2 +field\n";
}
